...
describe("GET /api/projects/:projectId/prompts/:promptId", () => {
  beforeEach(async () => {
    vi.clearAllMocks();
  ...
  it("returns a specific prompt by ID", async () => {
  ...
  it("returns 500 when service throws error", async () => {
    setAuthenticatedUser(1);
    getPromptByIdMock.mockRejectedValue(new Error("Database error"));
    ...
    const data = await response.json();
    expect(data).toEqual({ error: "Failed to get prompt" });
  });
