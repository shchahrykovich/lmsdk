...
export default function SpanWaterfall({ logs, onSpanClick }: SpanWaterfallProps) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-6">
      ...
      <div className="space-y-1">
        {/* Render groups and single spans in chronological order */}
        {[...groupedSpans.map(g => ({ type: 'group' as const, data: g })), ...singleSpans.map(s => ({ type: 'single' as const, data: s }))]
          .sort((a, b) => {
          ...
          .map((item, _idx) => {
            if (item.type === 'group') {
              ...
              >
                ...
                {/* Group summary/timeline - Right column */}
                {isCollapsed ? (
                    ...
                    <div className="relative h-8 bg-muted/30 rounded flex-1">
                      ...
                      <div
                        ...
                        style={{
                          ...
                          width: `${Math.max(totalDuration > 0 ? ((group.relativeEnd - group.relativeStart) / totalDuration) * 100 : 0, 0.5)}%`,
                        }}
                      ...
                      {hoveredGroup === groupKey && (
                        <div
                          ...
                          style={{
                            left: `${Math.min(totalDuration > 0 ? (group.relativeStart / totalDuration) * 100 : 0, 70)}%`,
                            top: "100%",
                            marginTop: "8px",
                          }}
                        >
                          <div className="space-y-1.5">
                            ...
                            <div className="text-xs text-muted-foreground">
                              Version {group.version} · {group.spans.length} execution{group.spans.length !== 1 ? 's' : ''}
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                              {group.errorCount > 0 ? (
                                <>
                                  ...
                                  <span className="text-red-600">{group.successCount} success, {group.errorCount} error{group.errorCount !== 1 ? 's' : ''}</span>
                                </>
                              ) : (
                                <>
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    {group.errorCount > 0 && (
                      <span className="text-red-600">{group.errorCount} error{group.errorCount !== 1 ? 's' : ''}</span>
                    )}
                  </div>
                )}
              </div>

              {/* Group spans - only shown when expanded */}
              {!isCollapsed && group.spans.map((span, index) => {
                ...
                return (
                  <div key={span.log.id} className="grid grid-cols-[250px_1fr] gap-4 items-center pl-6">
                    ...
                    <div className="relative flex items-center gap-2">
                      <div className="relative h-6 bg-muted/30 rounded flex-1">
                        ...
                        <div
                          ...
                          onMouseEnter={() => setHoveredSpan(spanGlobalIndex)}
                        ...
                        {isHovered && (
                          ...
                          >
                            <div className="space-y-1.5">
                              ...
                              {!span.log.isSuccess && span.log.errorMessage && (
              ...
              // Render single span (not grouped)
              ...
              return (
                <div key={span.log.id} className="grid grid-cols-[250px_1fr] gap-4 items-center">
                  {/* Span label - Left column */}
                  <div className="flex items-center gap-2 text-sm">
                    <div className="flex items-center gap-1.5 min-w-0 flex-1">
                      {/* Spacer to align with grouped items that have chevrons */}
